...
const Book = () => {
  ...
  const BookList = [
    ...
    {
      imgBook: "/images/book/propaganda.jpg",
      title: "Propaganda",
      status: "CURRENTLY READING",
      category: "Education",
  ...
  const completedBooks = BookList.filter(
    (book) => book.status === "COMPLETE"
  ).length;
  const currentlyBooks = BookList.filter(
      ...
      <div className="grid justify-center items-center">
        ...
        <div className="grid justify-center text-center gap-2 m-4 md:grid md:grid-cols-2 lg:grid-cols-3 grid-cols-2">
          {filteredBooks.map((book, index) => (
            <div
              key={index}
              className="grid justify-center border-2 p-2 rounded-lg"
            >
              <div className="flex justify-center items-center">
                <img
                  src={book.imgBook}
                  alt={book.title}
                  className="sm:h-96 w-full rounded-lg"
                />
              </div>
